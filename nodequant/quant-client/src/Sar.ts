...
export class Sar {
    ...
    public update(tick: Ticket) {
        ...
        const newExtreme = newTrend === 'falling'
            ? Math.min(this.extreme, tick.lowPrice)
            : Math.max(this.extreme, tick.highPrice);

        this.acc = newTrend !== this.trend
            ? 0.02
            : ( newExtreme === this.extreme ? this.acc : Math.min( this.acc + 0.02, 0.2) );
