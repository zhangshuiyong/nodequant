 ...
 function _registerEvent(myEngine) {
     ...
     global.AppEventEmitter.on(EVENT.OnAllConfigClientReadyed,function (msg) {
         ...
         global.AppEventEmitter.on(EVENT.OnTick,function (tick) {
             ...
             try{
                 global.NodeQuant.MainEngine.contractDic['CTP'][tick.symbol].ticket = tick;
 
                 const sortContract = (param) => {
                 
                     if(global.topFive[param].length > 0){
                         if (tick[param] > global.topFive[param][4].ticket[param] && global.topFive[param].filter(c=>c.symbol === tick.symbol).length <=0) {
                             global.topFive[param].push(global.NodeQuant.MainEngine.contractDic['CTP'][tick.symbol]);
                             global.topFive[param].sort((b,a) => a.ticket[param]-b.ticket[param]);
                         }
                     }
                         global.topFive[param] = global.topFive[param].slice(0,5);
                 ...
                 if( tick.closePrice >= tick.preClosePrice){
                     tick.increase = (tick.closePrice - tick.preClosePrice)/tick.preClosePrice;
                     tick.decrease = 0;
                 }
                 if( tick.closePrice < tick.preClosePrice){
                     tick.decrease = (tick.preClosePrice - tick.closePrice)/tick.preClosePrice;
                     tick.increase = 0;
                 }
                 sortContract('increase');
                 sortContract('decrease');
         ...
         let allContract=global.NodeQuant.MainEngine.GetAllContract();
         const symbols = Object.keys(allContract.CTP);
         for(let i in symbols){
             myEngine.Subscribe('CTP', symbols[i]);
         }
     });
