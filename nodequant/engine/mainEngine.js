...
class MainEngine{
    ...
    RecordError(error)
    {
        global.Application.RedisDBClient.rpush(System_Log_DB,JSON.stringify(error),function (err,reply) {
            if(err) {
    ...
    RecordLog(log){
        ...
        global.Application.RedisDBClient.rpush(System_Log_DB,JSON.stringify(log),function (err,reply) {
            if(err) {
